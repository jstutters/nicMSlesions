...
def train_cascaded_model(model, train_x_data, train_y_data, options):
    ...
    if options["full_train"] is False:
        ...
        model[0] = define_training_layers(
            model=model[0],
        ...
        for it in range(0, max_epochs, 10):
            ...
            model[0] = fit_model(model[0], X, Y, options, initial_epoch=it)
    ...
    X, Y, sel_voxels = load_training_data(
        train_x_data, train_y_data, options, model=model[0]
    )
    print("> CNN: train_x ", X.shape)
    ...
    if options["full_train"] is False:
        ...
        patience = 0
        best_val_loss = np.Inf
        model[1] = define_training_layers(
            model=model[1],
            num_layers=options["num_layers"],
            number_of_samples=X.shape[0],
        )
        ...
        for it in range(0, max_epochs, 10):
            options["max_epochs"] += 10
            model[1] = fit_model(model[1], X, Y, options, initial_epoch=it)
...
def test_cascaded_model(model, test_x_data, options):
    ...
    exp_folder = os.path.join(
        options["test_folder"], options["test_scan"], options["experiment"]
    )
    ...
    if np.sum(t1) > 0:
        ...
        t2 = test_scan(
            model[1], test_x_data, options, save_nifti=True, candidate_mask=(t1)
        )
    else:
    ...
    out_segmentation = post_process_segmentation(
        t2, options, save_nifti=True, orientation=flair_image.affine
    )
...
def load_training_data(
    train_x_data, train_y_data, options, model=None, selected_voxels=None
):
    """
...
def load_train_patches(
    ...
    nolesion_masks = [
        np.logical_and(np.logical_not(lesion), brain)
    ...
    for l_centers, nl_centers, image, lesion in zip(
        lesion_centers, nolesion_centers, images_norm, lesion_masks
    ):
        ...
        if balanced_training:
            if len(l_centers) > 0:
                ...
                idx = np.random.permutation(range(0, len(nl_centers))).tolist()[
                    : len(l_centers)
                ]
...
def load_test_patches(
    test_x_data, patch_size, batch_size, voxel_candidates=None, datatype=np.float32
):
    """
    ...
    if voxel_candidates is None:
        flair_scans = [test_x_data[s]["FLAIR"] for s in scans]
        selected_voxels = [
            get_mask_voxels(mask) for mask in select_training_voxels(flair_scans, 0.5)
        ][0]
    else:
...
def test_scan(model, test_x_data, options, save_nifti=True, candidate_mask=None):
    ...
    batch, centers = load_test_patches(
        test_x_data, options["patch_size"], options["batch_size"], candidate_mask
    )
    if options["debug"] is True:
    ...
    if check_min_error(seg_image, options, flair_image.header.get_zooms()):
        if options["debug"]:
            print(
                "> DEBUG ", scans[0], "lesion volume below ", options["min_error"], "ml"
            )
        seg_image = np.zeros_like(flair_image.get_data().astype("float32"))
...
def check_min_error(input_scan, options, voxel_size):
    ...
    num_elements_by_lesion = ndimage.labeled_comprehension(
        t_segmentation, labels, label_list, np.sum, float, 0
    )
...
def select_voxels_from_previous_model(model, train_x_data, options):
    ...
    for scan, s in zip(train_x_data.keys(), range(len(scans))):
        ...
        if options["debug"]:
            ...
            tmp_seg = nib.Nifti1Image(seg_mask, affine=flair.affine)
            tmp_seg.to_filename(
                os.path.join(
                    options["weight_paths"],
                    options["experiment"],
                    ".train",
                    scan + "_it0.nii.gz",
                )
    ...
    seg_mask = [
        im > 2 if np.sum(seg) == 0 else seg for im, seg in zip(images_norm, seg_masks)
    ]
...
def post_process_segmentation(
    input_scan, options, save_nifti=True, orientation=np.eye(4)
):
    """

...
def transform(Xb, yb):
    ...
    augmented_x = np.stack([rotate_180, rotate_0_flipped, rotate_180_flipped], axis=1)

    # select random indices from computed transformations
    r_indices = np.random.randint(0, 3, size=augmented_x.shape[0])

    Xb[indices] = np.stack(
        [augmented_x[i, r_indices[i], :, :, :, :] for i in range(augmented_x.shape[0])]
    )
...
def cascade_model(options):
    ...
    if not os.path.exists(
        os.path.join(options["weight_paths"], options["experiment"], "nets")
    ):
        os.mkdir(os.path.join(options["weight_paths"], options["experiment"], "nets"))
    if options["debug"]:
        if not os.path.exists(
            os.path.join(options["weight_paths"], options["experiment"], ".train")
        ):
            os.mkdir(
                os.path.join(options["weight_paths"], options["experiment"], ".train")
            )
    ...
    # save weights
    net_model = "model_1"
    net_weights_1 = os.path.join(
        options["weight_paths"], options["experiment"], "nets", net_model + ".hdf5"
    )
    ...
    if options["full_train"] is False:
        ...
        print(
            "> CNN: Loading pretrained weights from the",
            options["pretrained_model"],
            "configuration",
        )
        pretrained_model = os.path.join(
            options["weight_paths"], options["pretrained_model"], "nets"
        )
        ...
        if not os.path.exists(model):
            ...
            shutil.copyfile(os.path.join(pretrained_model, "model_1.hdf5"), net1_w_def)
            shutil.copyfile(os.path.join(pretrained_model, "model_2.hdf5"), net2_w_def)
        try:
...
def fit_model(model, x_train, y_train, options, initial_epoch=0):
    ...
    y_train = keras.utils.to_categorical(y_train == 1, len(np.unique(y_train == 1)))
    ...
    h = model["net"].fit_generator(
        da_generator(x_train_, y_train_, batch_size=batch_size),
        ...
        callbacks=[
            ModelCheckpoint(
                model["weights"], save_best_only=True, save_weights_only=True
            ),
            EarlyStopping(
                monitor="val_loss",
                min_delta=0,
